const QUICK_SETTINGS = [
  {
    id: "popup-openNextToParent",
    key: "openNextToParent",
    default: true,
    storage: "sync",
  },
  {
    id: "popup-removeDuplicatesInSelection",
    key: "removeDuplicatesInSelection",
    default: true,
    storage: "sync",
  },
  {
  ...
  {
    id: "popup-useCopyHistory",
    key: "useCopyHistory",
    default: false,
    storage: "local",
  },
  {
    id: "popup-checkDuplicatesOnCopy",
    key: "checkDuplicatesOnCopy",
    default: true,
    storage: "sync",
  },
];
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const syncKeys = QUICK_SETTINGS.filter((c) => c.storage === "sync").map(
    (c) => c.key,
  );
  const localKeys = QUICK_SETTINGS.filter((c) => c.storage === "local").map(
    (c) => c.key,
  );
  ...
  QUICK_SETTINGS.forEach((c) => {
    ...
    if (el) {
      ...
      el.addEventListener("change", (e) => {
        chrome.storage[c.storage]
          .set({ [c.key]: e.target.checked })
          .then(() => showStatus());
      });
    }
  });
  ...
  const [openBtn, copyBtn] = [
    document.getElementById("action-open-links"),
    document.getElementById("action-copy-links"),
  ];
